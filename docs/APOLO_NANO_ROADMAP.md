...
| C | M5 | 5.1 Otimização de treino | ✅ **2026-07-15 — +14,4% tok/s, medido em A/B controlado.** `cProfile` no loop de treino (preset `small`, 6L/256d) revelou o gargalo real: NÃO é a matmul (BLAS já cuida dela) — é o **GELU**, que sozinho comia ~36% do tempo (`gelu`+`gelu_backward` recalculando `tanh(...)` DUAS vezes por camada, uma no forward e outra no backward). `src/nanollm/layers.py`: `_gelu_fwd(x)` devolve `(y, tanh)` e o `MLP` guarda o `tanh` do forward para reusar no backward via `_gelu_bwd(x, t, dy)` — corta a metade redundante da conta. Bônus: `x**3`/`x**2` → `x*x*x`/`x*x` (evita o overhead do `np.power` elementwise). API pública `gelu`/`gelu_backward` preservada (delegam pros helpers) — os testes de gradcheck não mudam. **Medição honesta:** A/B de 60 passos cada, mesmo processo, mesma seed, alternando ANTIGO(git HEAD)/NOVO 2×: 530 tok/s → 606 tok/s = **+14,4%** (abaixo da meta informal de +30%, mas real e sem dependência nova — o profiling já tinha mostrado no M2.3 que o gargalo é banda de RAM para MATMUL; o GELU elementwise era a folga que sobrava). 33 testes de regressão (gradcheck+kvcache+train) verdes |
| C | M5 | 5.2 Quantização int8 | ✅ **2026-07-15** — `src/nanollm/quantize.py`: `quantize_int8`/`dequantize_int8` (simétrico, 1 escala por COLUNA — não por tensor inteiro, senão colunas de amplitude pequena perderiam precisão demais). `GPT.save_quantized()` grava só as matrizes 2D (embeddings + lineares — o que domina o tamanho) em int8; bias/LayerNorm (1D, já pequenos) seguem float32. **`GPT.load()` dequantiza sozinho e transparente** — quem chama (engine, generate.py) não muda uma linha. CLI `python -m src.nanollm.quantize --ckpt <pasta> [--data <pasta com val.npy>]` gera `model_q.npz` + relatório tamanho/ppl antes-depois. **Medido (não presumido):** checkpoint pequeno de teste ficou <60% do tamanho float32; degradação de perplexidade num round-trip real (20 passos de treino + quantizar) ficou **abaixo de 2%** (o teto do DoD). 5 testes novos (erro de quantização, coluna nula, correlação de logits, tamanho em disco, degradação de ppl) |
| C | M5 | 5.3 Corpus 2.0 (auto-retreino) | ✅ **2026-07-15 — fechado.** O pipeline "o Apolo aprende → o corpus cresce → o Nano re-treina" roda sozinho (`FLYWHEEL_HOUR`/M25.3) e agora tem DUAS fontes independentes, cada noite: `run_nightly_flywheel(..., source=)` ganhou o parâmetro — `"title"` (o professor rotula, como sempre) e `"reactions"` (os 👍 do Leo já são o rótulo, sem chamar o professor de novo — `distill.py::run_reaction_distillation`). Isoladas de propósito (lição do M14.2: nunca misturar distribuições) — datasets, candidatos e portão de qualidade SEPARADOS por fonte. `app.py::_run_flywheel_cycle(source)` roda as duas em sequência na madrugada, cada uma protegida (uma falhar não impede a outra). **Bug real achado e corrigido no processo:** as duas rodadas da mesma noite podiam cair no mesmo segundo e colidir no mesmo diretório de trabalho, uma sobrescrevendo o dataset/candidato da outra — o nome da pasta agora inclui a fonte (`{stamp}_{source}`). CLI: `python -m src.nanollm.flywheel --source reactions`. 4 testes novos (não chama o professor, pula com poucos pares, datasets isolados — o teste que pegou o bug de colisão). Suite → 1519 |
| C | M6 | 6.1 Experimento de escala | ✅ **2026-07-15 — medido, resultado HONESTO: `medium` (12,57M, 3,7× o `mini`) NÃO superou o `mini` no orçamento de tempo desta sessão.** Calibração real no corpus (231.497 tokens): `mini` 983 tok/s, `medium` 361 tok/s, `large` 127 tok/s — `medium` treina ~2,7× mais devagar por token, `large` ~7,7×. Treinei `medium` por 1500 passos (~75 min de máquina), mesmo harness/mesma metodologia (janelas sequenciais fixas) do baseline `mini` v1: **`medium` ppl 160,91 (nll 5,0809) vs `mini` ppl 157,96 (nll 5,0623) — `medium` ficou ~1,9% PIOR**, apesar de ter quase 4× mais parâmetros. Causa-raiz (não é bug, é o dado): o corpus tem só 231k tokens — `mini` já satura rápido e barato; `medium`, mais lento por passo, mal tinha começado a essa altura (a val loss ainda estava CAINDO a cada avaliação — 6.70→5.99→5.55→5.21→5.27→5.14→5.09→5.05 — sem sinal de overfit no corte, sugerindo que mais passos teriam fechado ou revertido a diferença). **Conclusão do experimento (decisão pelo dado, não pelo ego — o critério do próprio Épico 6.1):** neste corpus soberano pequeno, escalar parâmetros só compensa se vier junto de MAIS tempo de treino proporcional — não é escala "de graça". `large` nem foi tentado (7,7× mais lento tornaria o experimento inviável na janela desta sessão). Checkpoint `data/nanollm/ckpt_medium` fica salvo (gitignored) como referência; não promovido a produção. **Fecha o M6.1 com honestidade**, consistente com N4/§1 (velocidade de treino no CPU é o teto real) |
