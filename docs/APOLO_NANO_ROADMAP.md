...
| C | M5 | 5.3 Corpus 2.0 (auto-retreino) | 🔨 **parcial (2026-07-15, dentro do M-reações/Ano2 §5.3 do plano-mãe)** — o pipeline "o Apolo aprende → o corpus cresce → o Nano re-treina" já existe e roda sozinho (`FLYWHEEL_HOUR`/M25.3), mas faltava uma fonte de dado direto: `db.positive_reaction_pairs()` + `distill.py::run_reaction_distillation` transformam cada 👍 do Leo em par de treino SEM chamar o professor de novo (o veredito humano já é o rótulo) — dataset `answer_distill_reactions`, isolado do de título (lição do M14.2: não misturar distribuições). `FLYWHEEL_MIN_PAIRS` (env, default 5) destrava o ciclo com o volume real de hoje em vez do 12 fixo. Falta: religar esse dataset de reação DENTRO do `run_nightly_flywheel` automático (hoje é sourcing manual via CLI) — item pra um próximo incremento |
