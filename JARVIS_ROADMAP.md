...
| Q2 | M5 | Wake word (5.1) | 🔨 **fundação local** — `src/wake.py`: detecção da palavra de ativação ("Apolo"/"Jarvis") 100% local e determinística — normaliza (sem acento/pontuação), tolera preâmbulo ("ei/ok") e erro de transcrição (edit-distance ≤1), casa só no início (anti-falso-positivo) e extrai o COMANDO após a wake word. `GET /api/wake/config` + `POST /api/wake/detect`. Intenção `clock` fecha **"Apolo, que horas são?"** (verificado no preview ponta-a-ponta). Frontend: botão 👂 + escuta contínua (Web Speech) → detect → `/api/agency/ask` → fala; barge-in básico. **Pendência 🔒 HW:** captura contínua 100% soberana (openWakeWord/Whisper-loop) + latência CPU; 5.3 VAD. 15 testes |
