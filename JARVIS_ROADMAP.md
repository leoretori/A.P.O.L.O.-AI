...
| Q1 | M2 | MemoryFabric | 🔨 **fachada no ar** — `src/memory/` (MemoryFabric) unifica RAG (semantic) + base FTS/Supabase (knowledge) + lições do Coder (lesson) atrás de `remember(text, kind, tags)` / `recall(query, kind?)` → `MemoryHit` único; `/api/memory/recall` expõe a porta; injetado no runtime. Verificado no preview (recall unificado traz hits reais das 3 memórias). Falta: migrar os call-sites (chat/coder/agent) para o fabric |
