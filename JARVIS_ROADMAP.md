...
>
> **🐛 Correções críticas (2026-07-06):** (1) o **aprendizado não aprendia** — `fetch_page_text` referenciava um `headers` indefinido desde o refactor de perf (2200f13); NameError engolido por `except+logger.debug` mantinha `buscados:0`. (2) o **painel de aprendizado ficava congelado** — o SSE chamava `applyLearnStatus` inexistente sob `catch {}`. (3) botões 🕒 Atividade / 🔐 Permissões saíam brancos (IDs fora da regra CSS de nav). Todos com teste de regressão.
