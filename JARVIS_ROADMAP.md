...
| Q2 | M4 | Briefing diário | ✅ — `src/briefing.py` compõe um resumo FALÁVEL (o que aprendi na janela + o que fizemos/episódios + agenda de estudos + pendências) num `text` PT-BR determinístico; `GET /api/briefing?hours=`. Proativo: o `_scheduler_loop` envia o briefing como notificação uma vez por manhã (`BRIEFING_HOUR`, padrão 8h; -1 desliga). Frontend: botão 📻 no topo → busca e FALA o briefing. Verificado no preview (endpoint + botão + toast) |
