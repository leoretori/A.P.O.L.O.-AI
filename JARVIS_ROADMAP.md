...
| Q1 | M2 | Memória episódica | 🔨 **fundação + recall temporal** — tabela `episodes` (conversa resumida e DATADA) + `EpisodicMemory` (`src/memory/episodic.py`): resume uma sessão num episódio (LLM) e recupera por janela de tempo, traduzindo frases naturais ("ontem", "semana passada", "últimos N dias") em intervalos (`parse_when`). No fabric via `recall_when(frase)` + kind `episode`; endpoints `GET /api/memory/episodes?when=` e `POST /api/memory/episodes/record`. **DoD batida**: "o que fizemos ontem?" retorna episódio correto e datado (testado). Falta: gatilho automático (gravar episódio ao fim da conversa) — vem no Épico 2.3 |
