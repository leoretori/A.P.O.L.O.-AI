...
| Q1 | M2 | MemoryFabric | 🔨 **fachada no ar + 1º call-site migrado** — `src/memory/` (MemoryFabric) unifica RAG (semantic) + base FTS/Supabase (knowledge) + lições do Coder (lesson) atrás de `remember(text, kind, tags)` / `recall(query, kind?)` → `MemoryHit` único; `/api/memory/recall` expõe a porta; injetado no runtime. `agent_recall` (Modo Agente + CONSULTAR do Coder) já passa pelo fabric (com fallback sobre rt.rag). Verificado no preview (recall unificado traz hits reais das 3 memórias). Falta: migrar o recall do chat (RAG+FTS paralelo) e o bloco de lições do Coder |
