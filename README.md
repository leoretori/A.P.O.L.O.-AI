...
- ✅ **👤 Modelo profundo do Leo (M16.1 + 16.2)** — o que o Apolo entende sobre VOCÊ é um modelo **estruturado por categoria**: metas (curto/longo prazo), projetos ativos, hábitos, pessoas, preferências e valores. Curado por você (add/editar via `PATCH`/remover), migra perfis antigos sozinho, entra no system prompt **agrupado por seção**. A extração das conversas (`src/profile_extract.py`, determinística) **não grava nada sozinha** — propõe candidatos que você confirma ou recusa (`GET/POST /api/profile/candidates`), princípio "nada assumido". Painel "Sobre mim" + `/remember <fato>`
