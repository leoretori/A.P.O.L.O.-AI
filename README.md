...
- ✅ **👤 Modelo profundo do Leo (M16.1)** — o que o Apolo entende sobre VOCÊ deixou de ser lista rasa e virou modelo **estruturado por categoria**: metas (curto/longo prazo), projetos ativos, hábitos, pessoas, preferências e valores. Curado por você (add/editar via `PATCH`/remover), migra perfis antigos sozinho, e entra no system prompt **agrupado por seção** (`data/user_profile.json`). Painel "Sobre mim" + `/remember <fato>`
