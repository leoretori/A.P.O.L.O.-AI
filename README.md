...
- ✅ **👁️ Visão útil — ler tela & documentos (M22.1)** — *"o que tem nessa tela / nesse documento?"*: `src/vision_read.py` captura a **tela** (screenshot local via Pillow), extrai texto de **PDF/DOCX/texto**, e descreve **imagens/tela** via o modelo de visão. O conteúdo lido pode ser **guardado na memória** (RAG + base) num clique. Console **👁️ Visão útil** no painel 🛠️ Ações; `GET /api/vision/status`, `POST /api/vision/{screen,document}`. É honesto sobre o que dá agora (`capabilities()`): captura sempre; descrever imagem exige um modelo de visão (com o motor soberano text-only, precisa de um GGUF multimodal ou llava no Ollama)
- ✅ **🖠️ Ações de sistema com desfazer (M21.1)** — a primeira ação **nativa** (fora do texto): **mover/organizar arquivos** no disco, no mesmo rigor **prévia → confirmar → desfazer** e confinado às pastas que você autorizar (`files.move`, reusa o grant `files.write`). É a tarefa de sistema mais **reversível** — desfazer é mover de volta; nunca sobrescreve um destino existente. Console **📦 Mover / organizar** no painel 🛠️ Ações
- ✅ **🖱️ Navegador interativo em sandbox (M20.1/20.2)** — a automação web sobe de **leitura** para **interação**: `src/webtask.py` ganha `click`/`fill`/`submit` com o driver **Playwright** (🔒 opt-in), mantendo o padrão soberano (núcleo determinístico + driver injetável nos testes). Duas travas: a mesma **sandbox de domínios** checada em cada navegação, e o **gate de efeito** — passos que *mudam algo* (enviar formulário) têm **prévia de cada passo** e só rodam com sua **confirmação explícita** ("nunca um clique cego"). Cada ação com efeito entra numa **trilha durável e auditada** (🧾). Novo escopo `browser.interact` + console no painel **🛠️ Ações**. `POST /api/webtask/interactive/{plan,run}`, `GET .../trail`. *(20.3 — automatizar uma tarefa semanal real — aguarda instalar o Playwright e escolher a tarefa)*
- ✅ **🚀 Execução supervisionada de projetos (M19 completo)** — os projetos autodirigidos (M12) deixam de só ser *propostos*: o A.P.O.L.O. agora **executa os passos** — `src/project_exec.py` dá a cada passo um executor real (medir qualidade de síntese, limpar duplicatas...) no contrato de dois passos do M10 (**preview → executar**), embrulhando funções que já existem e são testadas. O **plano multi-passo** roda sozinho os passos seguros e **para num checkpoint** para você confirmar cada passo sensível (idempotente e retomável — o que já foi feito é pulado). E fecha o loop **propõe → faz → mede**: cada projeto guarda a métrica que o motivou (baseline no adopt) e, ao concluir, **re-mede e mostra o antes→depois** ("📈 melhorou — antes 12 → agora 0"). Tudo no painel **🎯 Projetos**. `GET /api/projects/{id}/plan`, `POST .../steps/{key}/preview|run`, `POST .../plan/run`, `GET .../outcome`
- ✅ **📜 Memória relacional & linha do tempo (M18 completo)** — os episódios datados (M2) deixam de ser texto solto e passam a saber **QUEM/O QUÊ/QUANDO** tocaram: `src/timeline.py` liga cada episódio às **entidades** do seu modelo (pessoas, projetos, metas) por nome próprio ou conceitos em comum (via `src.graph`, sem LLM). Com isso o Apolo responde perguntas **relacionais e datadas** — *"onde parei no projeto Y?"*, *"o que a Maria me pediu?"* — achando o episódio mais recente e respondendo com a data (integrado ao chat como fonte de verdade + `GET /api/recall`). O painel **📜 Linha do tempo** mostra a linha do tempo com chips de entidade e um **quem-é-quem** (cada pessoa: última vez, projetos e com quem coaparece). `GET /api/timeline`, `/api/people`
- ✅ **🔮 Antecipação útil (M17.3)** — o Apolo nota o que você anda **negligenciando** e sugere retomar na hora certa: `src/anticipation.py` acha as metas/projetos ativos que a atividade recente NÃO tocou (via `src.graph`) e propõe retomá-los, ancorado nos seus **hábitos** ("você costuma estudar de manhã — que tal retomar a meta X?"). Entra no briefing e no `GET /api/anticipations`. É seletivo: não cobra o que você já está tocando, só o esquecido
- ✅ **🎚️ Ritmo & tom que se adapta (M17.2)** — o Apolo ajusta **quanto** fala ao SEU estilo: `src/style.py` lê as suas preferências do perfil e deriva um tom — **direto**, **detalhado** ou **equilibrado** — que vira uma diretriz no system prompt (determinístico, sem LLM). Diga "prefiro respostas diretas" e ele passa a ir ao ponto; "gosto de explicações detalhadas" e ele aprofunda. Transparente e reversível (`GET /api/style` mostra o tom; mude a preferência e o tom muda)
- ✅ **👤 Modelo profundo do Leo (M16 completo)** — o que o Apolo entende sobre VOCÊ é um modelo **estruturado por categoria**: metas (curto/longo prazo), projetos ativos, hábitos, pessoas, preferências e valores; entra no system prompt **agrupado por seção**. A extração das conversas (`src/profile_extract.py`, determinística) **não grava nada sozinha** — propõe candidatos que você confirma ou recusa ("nada assumido"). O painel **"Sobre mim / Quem eu acho que você é"** mostra o modelo agrupado, os candidatos pendentes (✓/✕) e um **editor inline** (✎) para corrigir texto, recategorizar e ajustar horizonte — perfil profundo, **curado por você**. `/remember <fato>` no chat
