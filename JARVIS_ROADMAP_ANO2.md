...
>
> **Testes do 22.1 FECHADOS (2026-07-15):** a dívida ficou paga — `tests/test_vision_read.py` (19 casos: captura com resize real via `PIL.Image.new`, erro de captura, roteamento por tipo em `read_document` incl. o caminho REAL de `python-docx` não instalado — sem lib mockada, o ambiente já não tem, então o teste exercita o `ImportError` de verdade —, truncamento de texto grande, `describe_image` sem modelo/sucesso/erro, `capabilities()` honesto) + `tests/test_routers_vision.py` (12 casos: `/api/vision/status\|screen\|document`, captura falha não tenta descrever, `remember` grava/não-grava/sem-ingestor, base64 inválido). 31 testes novos, todos verdes.
>
> **🏁 Épico 22.2 ENTREGUE (2026-07-15):** visão + agência, com as travas do M6. Diferença chave: o console 👁️ do 22.1 é um clique SEU (consentimento por si só); 22.2 é o AGENTE decidindo, sozinho, olhar a tela — por isso passa pelo MESMO portão de permissão das demais ações do mundo. `src/tools/vision.py` registra a ferramenta `vision.screen` (escopo novo, negado por padrão) que captura + descreve via o modelo de visão; o base64 da captura NUNCA entra no resultado/auditoria — só tamanho e descrição textual. `src/tools/intent.py` ganhou a detecção ("veja minha tela", "o que tem na tela agora", "screenshot", "o que você vê") → `vision.screen`, formatada em texto natural (descrição, ou aviso honesto se faltar modelo de visão, ou erro de captura). Zero mudança de rota: `/api/agency/ask` e `/api/tools/run` já eram genéricos, só precisaram da ferramenta registrada. 22 testes novos (`test_tools_vision.py`: negado/auditado/captura+descreve/describe=false/falha de captura/sem modelo de visão; `test_tools_intent.py`: detecção, formatação, ponta-a-ponta negado/autorizado via `/api/agency/ask`).
>
> **🏁 Épico 22.3 ENTREGUE — M22 COMPLETO (2026-07-15):** câmera, opt-in de verdade. `vision_read.capture_camera()` tira UMA foto (abre a câmera, lê 1 frame, libera na hora — nunca fica ligada além disso, o mínimo de captura pro consentimento fazer sentido); requer `opencv-python` (import preguiçoso, 🔒 opt-in — igual ao Playwright do M20: erro claro em vez de quebrar o app). Console manual (clique seu = consentimento): botão **📷 Tirar uma foto** ao lado do 📸 no painel Ações, `POST /api/vision/camera` — VERIFICADO AO VIVO no preview: clicar mostra "captura de câmera requer: pip install opencv-python" (a lib não está instalada nesta máquina; a mensagem é exatamente a honesta esperada). Agência (mesma disciplina do 22.2): ferramenta `vision.camera` com escopo PRÓPRIO (`vision.camera` ≠ `vision.screen` — o mundo físico é mais sensível que a tela, autorizar um NÃO libera o outro); `intent.py` detecta "tira uma foto"/"veja pela câmera"/"webcam"/"me veja" → `vision.camera` (frases ambíguas tipo "foto da minha tela" priorizam a tela, o caso mais comum e menos sensível). `capabilities()` ganhou `camera` honesto (`False` nesta máquina — opencv não instalado). 40 testes novos (`test_vision_read.py`: captura via cv2 falso injetado em `sys.modules`, câmera não encontrada, falha de leitura, `release()` sempre roda mesmo com exceção; `test_routers_vision.py`: endpoint manual; `test_tools_vision.py`: escopo isolado do vision.screen; `test_tools_intent.py`: detecção/formatação/ponta-a-ponta). **DoD do M22 batido: o Apolo entende e age sobre conteúdo visual real (tela, documentos e câmera), integrado à memória e à agência com consentimento.** Suite 1458 → 1480.
...
> **🏁 Épico 23.1 ENTREGUE (2026-07-15):** contexto contínuo — a agenda passa a ser acompanhada, não só consultada. `src/presence.py` (determinístico, sem LLM): `heads_up_due(events, now, already_notified, lookahead_min)` é PURA — filtra eventos que começam dentro da janela (padrão 15 min) e ainda não avisados; `PresenceMonitor` guarda o "já avisei" entre ticks do scheduler (reinicia no restart, como `_last_briefing_date` e os outros marcadores — consequência aceitável, mesmo padrão do resto do app) e poda sozinho entradas de +1 dia. `format_heads_up` gera o texto falável ("⏰ Daqui a pouco (14:10): Reunião em Sala 3."). Ligado no `_scheduler_loop` (a cada 60s, junto dos lembretes/M4): reusa o `.ics` já autorizado em `calendar.read` (M6.3) — sem grant, fica silencioso (mesma regra de consentimento do M6); vira `db.add_notification(kind="reminder")`, então aparece no sino de notificações e pode ser falado como qualquer outro aviso. `PRESENCE_LOOKAHEAD_MIN` (env, padrão 15, -1 desliga). Evolui o briefing do M4 de "1x/dia" para "no momento certo", sem virar ruído (cada evento avisa uma vez só — o `PresenceMonitor` deduplica). 12 testes novos (`test_presence.py`: janela/limite/fora, dedup, evento sem start, pureza de `heads_up_due`, formatação com/sem local/título, `PresenceMonitor` avisa uma vez e poda). Suite 1480 → 1492. Faltam 23.2 (voz contínua, 🔒 HW — última milha do M3/M5) e 23.3 (modos de presença).
>
> **🏁 Épico 23.3 ENTREGUE (2026-07-15):** modos de presença. `src/presence.py` ganhou `current_mode(now)` (determinístico, sem LLM nem perfil): **descanso** (23h–7h, atravessa a meia-noite), **foco** (janela configurável do dia, padrão 9h–12h), **trabalho** (o resto) — janelas ajustáveis por kwargs, sem hardcode. `should_notify(kind, mode)` decide se um aviso sai AGORA: no modo descanso, avisos de baixa prioridade (`study`/`info`) ficam quietos — **"sem incomodar"**; o que é sensível a tempo (lembrete/briefing/agenda do 23.1) sempre avisa, não importa o modo. **Decisão de design deliberada:** o filtro NÃO entra em `db.add_notification` (o funil único de todos os chamadores) — `kind` default é `"info"` e o método é chamado em centenas de lugares/testes; filtrar ali tornaria QUALQUER teste que cria uma notificação dependente do RELÓGIO da máquina (achado ao revisar o design antes de implementar, não depois). Em vez disso, o scheduler calcula o modo 1x por tick e decide explicitamente antes de notificar estudo agendado — o estudo em si nunca para (aprender 24/7 continua sendo o comportamento certo), só o AVISO fica quieto. `GET /api/presence/mode` expõe o modo atual + rótulo (transparência, mesmo padrão do `/api/style`/`/api/nano/flywheel/diagnose`). 16 testes novos (`test_presence.py`: madrugada/foco/trabalho, limites inclusivos, janelas customizadas, `should_notify` silencia baixa prioridade só no descanso e nunca o sensível a tempo; `test_routers_learning.py`: endpoint). Suite 1492 → 1501. **Falta só 23.2** (voz contínua soberana, 🔒 HW) para fechar o M23 — depende de hardware de microfone/latência que este ambiente não tem.
...
> **🏁 M24 COMPLETO (2026-07-15) — fecha o Ano 2.** Os três épicos:
>
> **24.1 — loop fechado, de verdade "sozinho":** antes, `outcome()` (M19.3) só calculava o antes→depois quando o Leo clicava "📊 Medir resultado" — não era automático nem ficava guardado. Nova tabela `ProjectOutcome` (`src/storage_models.py`) + `save_project_outcome`/`list_project_outcomes` (`src/storage_projects.py`): cada medição vira uma LINHA permanente, não só um cálculo efêmero. `POST /api/projects/{id}/plan/run` (`routers/projects.py`), ao terminar com `status=="done"`, agora RE-MEDE sozinho e registra automaticamente (`auto=True`) + notifica o veredito ("melhorou"/"não melhorou", antes → depois) — nunca quebra a resposta do plano se a medição falhar. A chamada manual do botão "Medir resultado" segue existindo mas NÃO duplica a curva (é só consulta). `GET /api/projects/outcomes/history` expõe a série. 9 testes novos.
>
> **24.2 — placar de capacidade:** a infraestrutura É o `project_outcomes` do 24.1 — cada projeto concluído deixa uma prova medida (antes→depois, melhorou ou não) que se acumula com o tempo. **Honestidade deliberada:** não construí um gráfico fingindo "2 anos de série histórica" — esta base de código tem dias de idade real, não anos; um "placar de 2 anos" vazio seria decoração, não prova. O mecanismo (tabela + endpoint + contagem agregada na retrospectiva) está pronto e vai virar curva de verdade conforme o tempo passa e mais projetos rodam — é o que a retrospectiva do 24.3 já lê (`projects_measured: {total, improved}`).
>
> **24.3 — retrospectiva do Ano 2 + Ano 3:** `src/retrospective2.py` (mesmo padrão determinístico do M12.2, sem LLM) monta a narrativa falável do Ano 2 com números REAIS: tópicos aprendidos, cobertura do Nano (`nano_coverage`), win-rate da avaliação às cegas (M28), projetos medidos automaticamente (24.1) e se visão foi entregue (M22). Os temas do Ano 3 (`YEAR_THREE_THEMES`) não são inventados — vêm direto do que o roadmap (§10) já registrou como próximo passo real: crescer a cobertura do Nano, religar o dataset de reações no flywheel, compilar com Vulkan, decidir a GPU com dados do próprio caso. `GET /api/retrospective2`. Front: o painel Projetos ganhou um seletor **Ano 1 / Ano 2** ao lado do 🔊 Ouvir (Ano 2 é o padrão ao abrir) — VERIFICADO AO VIVO no preview: o texto do Ano 2 leu os números reais desta instância (19 tópicos, 3 dias ativos, cobertura do Nano 0% — honesto, esta instância local não tem histórico de takeover), e o toggle voltou pro texto do Ano 1 corretamente. 8 testes novos (`test_retrospective2.py` + `test_routers_retrospective.py`).
>
> Suite 1501 → 1515. **🎉 ANO 2 DO ROADMAP JARVIS — COMPLETO.** M13–M24 entregues em software (o único item não fechado é 23.2, voz contínua soberana, que é genuinamente `🔒 HW` — precisa de hardware de microfone real que este ambiente de desenvolvimento não tem). O Ano 2 fez o cérebro começar a ser seu (Nano integrado, roteamento, flywheel, quantização, experimento de escala honesto), aprofundou o modelo do Leo (perfil estruturado, personalização, memória relacional), deu ao Apolo agência que conduz (execução supervisionada, navegador interativo, arquivos com undo) e presença ambiente (visão útil + câmera + agenda antecipada + modos de presença). O Ano 3 (§10) já estava em andamento antes mesmo do M24 fechar — a retrospectiva reporta os números reais desse progresso, não uma promessa.
...
- **M27 — Takeover progressivo & medição.** O roteador migra tarefas ao Nano conforme o portão de qualidade libera; painel mostra a **% do cérebro que já é próprio** subindo. **DoD:** uma família de tarefas (ex.: toda a organização de conversas — títulos, tags, setores) 100% no Nano, Qwen só no chat aberto.
  - **🔨 M27 ENTREGUE (2026-07-11):** o arcabouço do takeover, medido. `src/nanollm/routing.py::route_task` (Nano primeiro → professor no fallback, exceção nunca derruba) + `task_enabled` (portão deliberado por família via `NANO_TASKS_OFF`). Contador persistente: tabela `NanoServe` (agregado task×served_by) + `NanoRoutingMixin` (`nano_record_serve`/`nano_coverage`). A geração de título (`chat_common.py`) já registra quem serviu. `/api/nano/coverage` + o card Apolo-Nano na Saúde mostram **"Cérebro próprio: X% das tarefas"** (verificado ao vivo: 75% num seed). 10 testes (roteamento + cobertura, deterministas). Hoje só o **título** roda no Nano em produção (setor/tags ainda são o `classify_sector` determinístico — candidatos prontos p/ ligar quando o flywheel treiná-los). Métrica-mãe do Ano 3 agora é **visível e honesta**.
  - **🔨 M27 — gate BINÁRIO como 2ª aposta (2026-07-15, infraestrutura, não treinado):** o multi-classe de setor mediu 31,4% (M4.3) — abaixo do útil. Hipótese a testar: um modelo de 3,4M generaliza MELHOR uma pergunta de 2 classes do que 9. `taskdata.py`: `collect_binary_pairs(db, target_sector)` reusa a MESMA fonte do setor (`learned_topics` + `classify_sector`, sem heurística nova) mas reformula como "É {setor}? sim/não", **balanceado** (mesmo tanto de sim/não — senão "sempre não" vira solução preguiçosa) e levanta `ValueError` se qualquer classe não bater o mínimo (mesma honestidade do resto do projeto — não força treino em dado insuficiente). `tasks.py::nano_binary_classify` faz a inferência com portão de qualidade (só aceita sim/não claros). **Por que só infraestrutura:** medido o banco REAL desta instância antes de treinar — só 19 tópicos com setor válido no total (vs. os 471 do M4.3 original), longe do mínimo pra um resultado significar algo. Decisão do Leo (2026-07-15): construir o pipeline testado com fakes agora, treinar de verdade quando o `learned_topics` crescer (o aprendizado autônomo já roda 24/7 — é questão de tempo, não de código). CLI: `python -m src.nanollm.taskdata --task binary --target-sector backend_apis`. 11 testes novos (dataset balanceado/determinístico/poucos-pares, inferência sim/não/tokens-extras/indisponível).
- **M28+ — Rumo ao chat próprio.** Distilar um Nano maior para diálogo curto/factual ancorado na memória do Leo; medir contra o Qwen às cegas. Aqui a GPU dedicada, **se e quando vier**, multiplica — mas o caminho já está andando.
  - **🔨 M28 (arcabouço) ENTREGUE (2026-07-11):** (1) **destilação de resposta curta** — `distill.py::distill_answers` (professor responde curto/factual à pergunta real; `_valid_answer` barra código/markdown/lixo; template `Pergunta→Resposta`), a mesma máquina do M25 apontada a uma tarefa muito mais dura. (2) **avaliação ÀS CEGAS** — `src/nanollm/blind_eval.py::blind_compare`: Nano vs Qwen na mesma pergunta, **ordem A/B embaralhada por semente** e desfeita só no placar → o juiz não sabe quem é quem; `make_llm_judge` (juiz Qwen real) + `run_blind_eval` (banco+Nano+Qwen) + CLI `python -m src.nanollm.blind_eval`. Devolve o **win-rate honesto do cérebro próprio**. 7 testes (mapeamento apesar do shuffle, empate, parse robusto do voto). **Honestidade dura:** num Nano de 3,4M no CPU o win-rate será baixo — de propósito; o valor é ter a MÁQUINA e a MEDIÇÃO prontas para quando a escala (Nano maior via preset `large` + GPU) chegar. O número não mente.
  - **🔨 M28 (Q&A ancorado no conhecimento) ENTREGUE (2026-07-11):** o combustível certo p/ o Nano-de-diálogo, **isolado** do flywheel de título. `distill.py::source_knowledge_grounded_pairs` puxa as sínteses dos 7 agentes (`get_learning_history`) e o Qwen as vira pares **pergunta→resposta ancorados** (prompt P:/R:, `_parse_qa` tolerante); `run_knowledge_distillation` grava um dataset SEPARADO (`answer_distill_grounded`) no formato do fine-tune; CLI `python -m src.nanollm.distill --source knowledge`. **Decisão de projeto (a pedido do Leo):** título NUNCA mistura com o banco de conhecimento (reintroduziria o descasamento do M14.2); o banco alimenta só o chat/resposta. 5 testes novos (parse P:/R:, sourcing, ponta a ponta, sem-sínteses).
  - **📏 M28 medição real (2026-07-15):** rodei `python -m src.nanollm.blind_eval --limit 5` de verdade contra o checkpoint vivo (`ckpt_v1`, ainda o de título — nenhum fine-tune de resposta foi promovido). Resultado: **Nano 2 · Qwen 3 · win-rate 40,0%** nas 5 perguntas reais do banco. **Leitura honesta, não triunfalista:** com `n=5`, cada pergunta vale 20 pontos percentuais — a diferença para a medição anterior (20%, 1/5, sessão passada) é ruído de amostra pequena, não uma melhora real; nada mudou no modelo entre uma medição e outra. **Por que não fui além (fine-tune de resposta real):** medi o volume de dado disponível AGORA no banco de produção antes de treinar — `first_user_messages()` 5, `positive_reaction_pairs()` 0 (nenhum 👍 dado ainda, mesmo com o flywheel de reações religado no M5.3). Treinar com 5 exemplos seria decoreba pura, não aprendizado — a mesma lição do "poucos pares" de sempre. **Destravado para a próxima vez:** basta você continuar usando o app normalmente (abrindo sessões novas + dando 👍 nas respostas boas) — os dois flywheels (título e reações) já rodam sozinhos de madrugada assim que houver volume.
